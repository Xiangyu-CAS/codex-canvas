...
5. `Quick Edit`, `Remove BG`, `Expand`, `Edit Text`, and `Edit Elements` are implemented as background Agent-Canvas jobs. They create a canvas placeholder immediately, run Codex/ImageGen through the matching Agent-Canvas operation skill and bundled Codex App CLI, then replace the placeholder with the collected output.
...
7. `Edit Elements` asks ImageGen for a low-detail instance segmentation map, then Agent-Canvas locally splits the source image into four-channel transparent PNG object/text layers plus a residual background. Agent-Canvas immediately imports the residual background and transparent object/text layers, stacks them to the right of the source image so they reconstruct the original composition, and continues a background-completion pass from the original image plus residual background in the background. When completion finishes, Agent-Canvas replaces the existing background layer asset in place and refreshes that layer. Intermediate segmentation and raw completion images stay internal to the job and are not added to the canvas.
