...
1. Agent-Canvas first creates a padded PNG input using the selected scale, target ratio, and the user's final source-image position inside the preview frame. Treat the pasted original content as locked source content and the padded surroundings as the outpaint region.
