...
現在の Codex 会話で `@Codex-Canvas codex canvas を開いて` と入力すると、Codex-Canvas は in-app browser でプロジェクトのローカルキャンバスを開きます。左側でチャットを続けながら、右側で視覚素材を管理できます。Codex/ImageGen が画像を生成すると、Codex-Canvas はその画像を現在のプロジェクトに保存し、アクティブなキャンバスへ配置します。生成結果はすぐに編集可能なキャンバス素材になります。
...
- Codex/ImageGen の出力を現在のプロジェクトキャンバスに自動収集します。
