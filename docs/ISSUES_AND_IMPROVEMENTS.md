...
- Version grouping is available through CLI, HTTP, MCP, and the in-canvas discovery panel by grouping canvas objects on `sourceObjectId`, `batchId`, `layoutMode`, or `prompt`. The discovery panel can select and frame grouped versions for side-by-side comparison. Deeper visual diffing is still future work.
...
- Add thumbnail diffing or annotation overlays for grouped canvas outputs.
